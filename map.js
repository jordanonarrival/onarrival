...
async function showActivityModal(
  ...
  document.querySelector("[activity-data='activity_description']").textContent =
    clickedActivity._place._place_category.category_name ||
  ...
  if (foundObject) {
    ...
    document
      .querySelector("[wized='activity_like_button']")
      .removeAttribute("activity-data-id")
  }

  document.querySelector("[activity-data='image']").src =
    clickedActivity._place.oa_place_image.url

  document.querySelector("[activity-data='title']").innerText =
    clickedActivity._place.google_name
  ...
  document.querySelector("[activity-data='sub_title']").innerText =
    clickedActivity.note // Assuming 'note' is the subtitle
  ...
  const destinationName =
    clickedActivity._place._destination_activity.mobi_destination_name
  ...
  document.querySelector(
    "[activity-data='oh_monday']"
  ).textContent = clickedActivity._place.google_monday_hours
    ? clickedActivity._place.google_monday_hours
    : "Not added"

  document.querySelector(
    "[activity-data='oh_tuesday']"
  ).textContent = clickedActivity._place.google_tuesday_hours
    ? clickedActivity._place.google_tuesday_hours
    : "Not added"

  document.querySelector(
    "[activity-data='oh_wednesday']"
  ).textContent = clickedActivity._place.google_wednesday_hours
    ? clickedActivity._place.google_wednesday_hours
    : "Not added"

  document.querySelector(
    "[activity-data='oh_thursday']"
  ).textContent = clickedActivity._place.google_thursday_hours
    ? clickedActivity._place.google_thursday_hours
    : "Not added"

  document.querySelector(
    "[activity-data='oh_friday']"
  ).textContent = clickedActivity._place.google_friday_hours
    ? clickedActivity._place.google_friday_hours
    : "Not added"

  document.querySelector(
    "[activity-data='oh_saturday']"
  ).textContent = clickedActivity._place.google_saturday_hours
    ? clickedActivity._place.google_saturday_hours
    : "Not added"

  document.querySelector(
    "[activity-data='oh_sunday']"
  ).textContent = clickedActivity._place.google_sunday_hours
  ...
  function initializeActivityButtons() {
    ...
    document
      .querySelectorAll('[wized="activity_like_button"]')
      .forEach(element => {
        ...
        if (isManuallyChanged[parentPlaceId]) {
          console.log(
            `State manually changed for parentPlaceId: ${parentPlaceId}`
          )
        } else {
          console.log(
            `State not manually changed for parentPlaceId: ${parentPlaceId}`
          )

          // Update the favorite state based on the attribute 'activity-liked' from the element
          favoritesState[parentPlaceId] =
            element.getAttribute("activity-liked") === "true"
        }
        ...
        element.addEventListener("click", function() {
          console.log(
            `activity_like_button clicked for parentPlaceId: ${parentPlaceId}`
          )
          console.log(
            `Current favorite state before toggle: ${favoritesState[parentPlaceId]}`
          )
  ...
  window.Wized.push(async Wized => {
    ...
    function createMarker(activity, map, activeMarker) {
      ...
      const popupHTML = `
        ...
        <div class="text-size-small text-color-grey">${place._place_category
          .category_name || "No description available."}</div>
        </div>
        </div>
